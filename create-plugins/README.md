...
To localize script, we must `register` or `enqueue` script first.
...
Text domain is a unique identifier that you use to distinguish
the translation of your from those of other plugins.

Besides that, we need to use the `gettext` function to get translated text,
2 popular of them are:

- `__('string', 'text-domain')`: use when we want to get the raw translated text
- `_e('string', 'text-domain')`: use when we need to `echo` the translated text to the screen
- `esc_html__()`: to escape html and return raw string
- `esc_html_e()`: to escape html and echo to the screen

Then, we need a `pot` file (Portable Object Template), this file will
contain a list of all translatable strings, but it will not have
any translation text. That is, it will serve only as a template for
translation.

Usually, this `pot` template will be sent to a professional translator, and
from it, the translator then generates a file with the `.po` extension and
having this file enhance the translator. And then, we can generate the
`.mo` file. These 2 files contain all the translations.
 ...
 if (!class_exists('MV_Slider')) {
    class MV_Slider {
        ...
        public function define_constants() {
            ...
            define('MV_SLIDER_URL', plugin_dir_url(__FILE__));
            define('MV_SLIDER_VERSION', '1.0.0');
            define('MV_SLIDER_TEXT_DOMAIN', 'mv-slider');
        }
